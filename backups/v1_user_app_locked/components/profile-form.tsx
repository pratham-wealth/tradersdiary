...
export function ProfileForm({ initialProfile }: { initialProfile: ProfileData | null }) {
    ...
    const getPlanDisplay = () => {
        ...
        if (subscriptionStatus === 'trial') {
            return '7-Day Pass (Paid)'; // Or Handle logic if we ever support Paid Trial
        }
        if (planType === 'pro' && subscriptionStatus === 'active') {
            return 'Professional Plan (Active)';
        }
        return 'Starter Pass (Active)'; // Default to Starter Pass for 'free' or 'basic'
    };
